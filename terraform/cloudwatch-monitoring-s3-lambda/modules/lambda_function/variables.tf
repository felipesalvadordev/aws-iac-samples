...
variable "use_image" {
  description = "If true, deploy the Lambda as a container image using image_uri. If false, use zip/handler runtime." 
  type        = bool
  default     = false
}

variable "image_uri" {
  description = "URI of the container image (ECR) for the Lambda when use_image = true"
  type        = string
  default     = ""
}
