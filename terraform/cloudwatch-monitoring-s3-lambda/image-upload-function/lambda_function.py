...
def publish_metrics(function_name, processing_time, image_count, success):
    ...
    """
    try:
        # Prepare all metrics in a single MetricData list to minimize API calls
        metric_data = [
            {
        ...
        # Single API call for all custom metrics
        cloudwatch.put_metric_data(
            Namespace='ImageProcessor/Lambda',
            MetricData=metric_data
        )

        logger.debug(f"Published {len(metric_data)} custom metrics to CloudWatch (batched)")
